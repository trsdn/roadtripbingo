...
// Default point values for the scoring game mode
...
// Known game mode identifiers (used to distinguish ids from legacy display strings)
const GAME_MODE_IDS = ['classic', 'multihit', 'scoring'];
 ...
 * @param {string} options.gameMode - Game mode id ('classic'|'multihit'|'scoring') or legacy display string
 * @param {string} options.gameModeLabel - Display label for the game mode shown on cards
 * @param {string} options.gameDifficulty - Game difficulty level
 * @param {boolean} options.includeRulesPage - Whether to append a printable rules page
 * @param {Object} options.rulesContent - Localized rules content ({ title, sections })
 * @param {Object} options.scoreLabels - Localized labels for the score sheet page
...
async function generatePDF(options) {
  try {
    ...
    // to legacy display strings (older callers passed e.g. "Multi-Hit Mode (MEDIUM)")
    const cardHeaderMode = gameModeLabel !== undefined
      ? gameModeLabel
      : (GAME_MODE_IDS.includes(gameMode) ? '' : gameMode);
    ...
    if (includeRulesPage && rulesContent) {
      renderRulesPage(pdf, rulesContent);
    }
    if (gameMode === 'scoring') {
      renderScorePage(pdf, scoreLabels || {});
    }
...
/**
 * Set the font style if the jsPDF instance supports it (mocks may not)
 * @param {Object} pdf - jsPDF instance
 * @param {string} style - Font style ('normal'|'bold')
 */
function setFontStyle(pdf, style) {
...
/**
 * Render a printable rules page on a new PDF page
 * @param {Object} pdf - jsPDF instance
 * @param {Object} rulesContent - Localized content ({ title, sections: [{ heading, lines }] })
 */
function renderRulesPage(pdf, rulesContent) {
  const margin = 15;
  const maxY = 270; // mm - bottom limit before a page break
  const lineHeight = 5;
  ...
  pdf.text(rulesContent.title || '', margin, y);
  setFontStyle(pdf, 'normal');
  y += 10;

  const sections = rulesContent.sections || [];
  for (const section of sections) {
    // Page break if the heading would not fit anymore
    if (y > maxY) {
      pdf.addPage();
      y = margin + 5;
    }
...
/**
 * Render a printable score sheet page for the scoring game mode
 * @param {Object} pdf - jsPDF instance
 * @param {Object} scoreLabels - Localized labels ({ title, achievement, points, player, row, column, diagonal, fullHouse, total })
 */
function renderScorePage(pdf, scoreLabels) {
  const margin = 15;
  const playerCount = 4;
  const rowHeight = 12;
  const achievementColWidth = 70;
  const pointsColWidth = 25;
  const playerColWidth = 20;
  const tableWidth = achievementColWidth + pointsColWidth + (playerCount * playerColWidth);
  ...
  pdf.setTextColor(0, 0, 0);
  setFontStyle(pdf, 'bold');
  pdf.text(scoreLabels.title || 'Score Sheet', margin, y);
  setFontStyle(pdf, 'normal');
  y += 10;

  // Table rows: header, achievements, total, plus a few blank rows for custom rules
  const scoreRows = [
    { label: scoreLabels.row || 'Row', points: DEFAULT_SCORING.row },
    { label: scoreLabels.column || 'Column', points: DEFAULT_SCORING.column },
    { label: scoreLabels.diagonal || 'Diagonal', points: DEFAULT_SCORING.diagonal },
    { label: scoreLabels.fullHouse || 'Full House', points: DEFAULT_SCORING.fullHouse }
  ];
  const blankRowCount = 3;
  const totalRowCount = 1 + scoreRows.length + 1 + blankRowCount; // header + scores + total + blanks
  const tableTop = y;
  const tableHeight = totalRowCount * rowHeight;

  // Outer border
  pdf.setDrawColor(0, 0, 0);
  pdf.rect(margin, tableTop, tableWidth, tableHeight, 'S');

  // Horizontal grid lines
  for (let r = 1; r < totalRowCount; r++) {
    const lineY = tableTop + (r * rowHeight);
    pdf.line(margin, lineY, margin + tableWidth, lineY);
  }

  // Vertical grid lines
  const colX = [margin + achievementColWidth, margin + achievementColWidth + pointsColWidth];
  for (let p = 1; p < playerCount; p++) {
    colX.push(margin + achievementColWidth + pointsColWidth + (p * playerColWidth));
  }
  for (const x of colX) {
    pdf.line(x, tableTop, x, tableTop + tableHeight);
  }

  const textY = (rowIndex) => tableTop + (rowIndex * rowHeight) + (rowHeight / 2) + 1.5;
  const cellPadding = 2;

  // Header row
  pdf.setFontSize(10);
  setFontStyle(pdf, 'bold');
  pdf.text(scoreLabels.achievement || 'Achievement', margin + cellPadding, textY(0));
  pdf.text(scoreLabels.points || 'Points', margin + achievementColWidth + cellPadding, textY(0));
  for (let p = 0; p < playerCount; p++) {
    const playerX = margin + achievementColWidth + pointsColWidth + (p * playerColWidth);
    pdf.text(`${scoreLabels.player || 'Player'} ${p + 1}`, playerX + cellPadding, textY(0));
  }
  setFontStyle(pdf, 'normal');
  ...
  scoreRows.forEach((row, index) => {
    ...
    pdf.text(row.label, margin + cellPadding, textY(rowIndex));
    pdf.text(String(row.points), margin + achievementColWidth + cellPadding, textY(rowIndex));
  });

  // Total row (bold label, player columns stay empty for manual tallying)
  const totalRowIndex = scoreRows.length + 1;
  setFontStyle(pdf, 'bold');
  pdf.text(scoreLabels.total || 'Total', margin + cellPadding, textY(totalRowIndex));
  setFontStyle(pdf, 'normal');
  // Remaining blank rows are left empty for custom family rules
}
